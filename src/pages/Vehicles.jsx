...
export const Vehicles = () => {
    ...
    const { uid1 } = useParams();
    const { store, dispatch } = useGlobalReducer();
    const {loading, setLoading} = useState(true);
    const navigate = useNavigate();
    const id = parseInt(uid1)


    useEffect(() => {

       
        fetch(`https://swapi.tech/api/vehicles/${uid1}`)
            .then((resp) => resp.json())
            .then(data => {
                dispatch({ type: "get_vehicles", payload: data.result.properties })
                            ...
                            <div className="row align-items-center">
                                ...
                                <div className="col-md-8">
                                    ...
                                    <div style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
                                        <strong>Capacidad de carga:</strong> <span style={{ color: "#fff" }}>{store.vehicles.cargo_capacity}</span>
                                    </div>
                                    <div style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
                                        <strong>Pasajeros:</strong> <span style={{ color: "#fff" }}>{store.vehicles.passengers} </span>
                                    </div>
                                    <div style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
                                        <strong>Velocidad máxima:</strong> <span style={{ color: "#fff" }}>{store.vehicles.max_atmosphering_speed} km/h</span>
                                    </div>
                                    <div style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
                                        <strong>Longitud:</strong> <span style={{ color: "#fff" }}>{store.vehicles.length} m</span>
                                    </div>
                                    <div style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
                                        <strong>Precio:</strong> <span style={{ color: "#fff" }}>{store.vehicles.cost_in_credits} credits</span>
                                    </div>
                                    <div style={{ fontSize: "1.1rem" }}>
                                        <strong>Modelo:</strong> <span style={{ color: "#fff" }}>{store.vehicles.model}</span>
                                    </div>
                                </div>
                            </div>
